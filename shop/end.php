...
if (isset($_GET ['setAddress'])) {
	...
	$array_function = $_SESSION ['array_end'];
	
	$qtd_end = count($array_function);
	
	
	if ($id_end <= $qtd_end - 1 ) {
		
	$endereco_p = $array_function[$id_end];
...
<?php $in = 1; while ($in <= $qtd_end - 1) { ?>

<?php

	$endereco_p = $array_function[$in];
	$data_end = json_decode($endereco_p);
...
<div class="AddressBox">
   ...
   <a  href="end.php?setAddress=<?php echo $in ?>">
...
<?php $in++; } ?>
							  ...
							  <button type="button" onClick="verify_new_end(2)" class="continueButton" >Continuar</button>
